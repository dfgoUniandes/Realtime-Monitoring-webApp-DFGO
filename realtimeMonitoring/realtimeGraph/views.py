...
"""
Funcion para state pareja 02 - Grupo 07
"""

def get_data_state(request, **kwargs):

    state_param = kwargs.get("id_state", None)
    ...
    data_result["data"] = state_data

    
    print("user_param ",state_param)
    print("measure_param ",measure_param)

    
    data_by_state = Data.objects.filter(
        state=state_param,measurement__name=selected_measure.name,  time__gte=start.date(), time__lte=end.date())
    
    try:
        location = data_by_state[0].state.location
        print(location)
    except IndexError:
        return JsonResponse(data_result)
    ...
    min_val = data_by_state.aggregate(
        Min('value'))['value__min']
    max_val = data_by_state.aggregate(
        Max('value'))['value__max']
    avg_val = data_by_state.aggregate(
        Avg('value'))['value__avg']
    state_data.append({
...
"""
Funcion para traer data city - pareja 02 equipo 07

"""

def get_data_city(request, **kwargs):

    city_param = kwargs.get("id_city", None)
    measure_param = kwargs.get("measure", None)
    city_data= []
    if city_param == None or measure_param == None:
        return JsonResponse({"error":"No se porporcionó un id ciudad o medida"})
    try:
        selected_measure = Measurement.objects.filter(name=measure_param)[0]
    except IndexError:
        measurements = Measurement.objects.all()
    ...
    data_result["start"] = start_formatted
    data_result["end"] = end_formatted
    data_result["data"] = city_data

    
    print("user_param ",city_param)
    print("measure_param ",measure_param)

    start_ts = int(start.timestamp() * 1000000)
    end_ts = int(end.timestamp() * 1000000)

    data_by_city = Data.objects.filter(
        station__location__city=city_param,measurement__name=selected_measure.name,  time__gte=start, time__lte=end)
    try:
        location = data_by_city[0].station.location
    except IndexError:
        return JsonResponse(data_result)
    ...
    min_val = data_by_city.aggregate(
        Min('value'))['value__min']
    max_val = data_by_city.aggregate(
        Max('value'))['value__max']
    avg_val = data_by_city.aggregate(
        Avg('value'))['value__avg']
    city_data.append({
        'min': min_val if min_val != None else 0,
        'max': max_val if max_val != None else 0,
